...
Legato is not only a token, it's a whole ecosystem relying on 2 major components:

- **(token) LDOGE Legato:** A BEP-20 token, used to reward liquidity providers, stakers and holders.
- **(app) LegatoSwap:** It consists of multiple Decentralized Finance (DeFi) products deployed on the Binance Smart Chain.

LegatoSwap and LDOGE Legato features are interdependent and it's through their combination that the ecosystem makes sense.

*All Legato products are 100% open source, you can find the code on [github](https://github.com/lamboDoge/).*
...
The main innovation brought by LDOGE Legato is its tax management. Legato doesn’t take a tax on each transaction but on each buy or sell,
...
Legato also has a specific way to handle its tax, in order to reduce the price impact of each transaction,
...
LDOGE Legato has a max supply of 100,000,000,000 tokens. They are distributed through the dead address, liquidity pools, staking pools and
vaults that will be unlocked periodically to increase the reward rate of liquidity mining and staking pools,
and develop new products on LegatoSwap
...
- 8 billion tokens (8%) will be used to reward Legato stakers.
...
# DeFi with LegatoSwap

The main products provided by LegatoSwap are its swapping interface, and the staking platform.

## Swapping interface

On its first version, the swapping interface of LegatoSwap uses the PancakeSwap protocol.
Which allows you to trade any BEP-20 tokens with liquidity on PancakeSwap. But in comparison to the PancakeSwap interface,
LegatoSwap is much more user friendly and testnet compatible.

LegatoSwap also allows you to provide, manage and remove liquidity.

## Staking and liquidity mining

One of our goals is to keep the price up and increase the price floor of Legato. In order to achieve this,
we've added a staking platform to LegatoSwap.
...
At launch, LegatoSwap offers 2 staking pools, one "classic pool", and one "liquidity mining pool". Let's break down these concepts and explain
why it will help us to achieve our goals.

## Classic staking

The available "classic pool" on LegatoSwap allows users to stake their `LDOGE`, rewards them with `LDOGE`,
...
The available "liquidity mining pool" on LegatoSwap allows users to stake their `LP-LDOGE-BNB` tokens and rewards them with `LDOGE`.
